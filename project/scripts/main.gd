...
func _update_status() -> void:
	...
	if mode_label:
		...
		mode_label.add_theme_color_override(
			"font_color", Color.YELLOW if TerminalManager.is_mock_mode else Color.GREEN
		)

	if addon_label:
		addon_label.text = "ADDON: %s" % ("YES" if TerminalManager.is_addon_available else "NO")
		addon_label.add_theme_color_override(
			"font_color", Color.RED if not TerminalManager.is_addon_available else Color.CYAN
		)
